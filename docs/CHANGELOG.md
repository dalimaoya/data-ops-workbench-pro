...
## v4.5.0 — 2026-03-26 全版本缺陷修复 & 发布

本版本完成全版本补测，修复 11 个跨版本缺陷，覆盖 v3.4–v3.10 共 6 个版本。

### 缺陷修复（11 项）

| 编号 | 版本 | 修复内容 |
|------|------|----------|
| BUG-001 | v3.10 | 导入校验识别 `_操作` 列 DELETE 标记，支持删除行检测 |
| BUG-002 | v3.10 | 回写增加 DELETE 执行分支，支持模板标记删除行的实际删除 |
| BUG-003 | v3.10 | 前端差异预览增加 delete 类型红色标签和行样式 |
| BUG-004 | v3.10 | 回写结果页和预览摘要卡片展示删除行数 |
| BUG-005 | v3.9  | `browse_table_data` 接口返回 `allow_insert_rows` 字段 |
| BUG-006 | v3.5  | 主键不可变校验错误提示显示数据库中真实原始主键值 |
| BUG-007 | v3.9  | INSERT 模板预留行数从硬编码 50 改为可配置，默认 200 行 |
| BUG-008 | v3.6  | `get_import_diff` / `get_diff_report` / `get_export_info` 添加角色校验 |
| BUG-009 | v3.8  | launcher 版本比较改用语义化版本，仅远程大于本地时提示更新 |
| BUG-010 | v3.8  | `CURRENT_VERSION` 从 `version.txt` 或 `package.json` 自动读取，不再硬编码 |
| BUG-011 | v3.4  | `build.sh` 构建方式从 PyInstaller 统一为 Nuitka，与 CI 保持一致 |

### 发布配套
- 新增 `version.txt`（内容 `4.5.0`），供启动器和后端自动读取版本号
- 前端关于页面版本号更新为 v4.5.0
- CI 双平台编译（Linux x64 + Windows x64）

---

## v4.4.0 — 2026-03-25 AI 批量智能填充

### 后端
- 新增 `POST /api/ai/smart-fill` — 模式检测与填充建议
- 新增 `POST /api/ai/smart-fill/apply` — 确认填充（复用差异预览回写流程）
- 新增 `smart_fill_engine.py` — 四层模式检测引擎：
  - 数字递增检测（等差序列自动续填，支持非连续行）
  - 日期递增检测（多日期格式支持，自动保持格式一致）
  - 关联填充（字段间映射推断，如部门→负责人，置信度≥0.7 才使用）
  - 频率填充（该列历史最常见值，占比≥30% 才推荐）
  - LLM 增强（可选，复杂模式识别）
- 每条建议带置信度评分

### 前端
- 数据浏览页新增「🧠 AI 智能填充」按钮
- 新增 `AISmartFillModal` 组件，三步交互：
  1. 选择需要填充的列（仅可编辑字段）+ 可选 AI 增强
  2. 检测模式 → 展示各字段的检测结果和置信度
  3. 预览建议值，支持逐条/全选/仅高置信度三种采纳方式
- AI 填充值紫色高亮显示，置信度进度条可视化
- 确认后进入标准差异预览回写流程

---

## v4.3.0 — 2026-03-25 AI 操作日志智能分析

### 后端
- 新增 `POST /api/ai/log-analyze` — 操作日志智能分析
- 规则引擎：操作频次统计、异常模式检测（失败率、夜间操作、高频重复）、风险评估
- LLM 增强：自然语言分析摘要、问题溯源建议

### 前端
- 日志中心新增「🤖 AI 分析」按钮
- 分析结果面板：操作摘要、异常检测列表、风险评估、建议操作
- 支持选择时间范围分析

---

## v4.2.0 — 2026-03-25 AI 自然语言数据查询

### 后端
- 新增 `POST /api/ai/nl-query` — 自然语言转筛选条件
- 规则引擎：关键词匹配常见查询模式（等于/包含/大于/小于/时间范围）
- LLM 增强：复杂语义理解、多条件组合

### 前端
- 数据浏览页新增「🤖 AI 查询」按钮
- 自然语言输入框 + 筛选条件预览 + 一键应用到数据浏览
- 示例查询引导

---

## v4.1.0 — 2026-03-25 AI 导入数据智能校验

### 后端
- 新增 `POST /api/ai/data-validate` — 导入数据智能校验
- 四类检测：异常值（统计离群）、格式异常（模式偏差）、重复值、跨字段逻辑冲突
- 规则引擎兜底 + LLM 增强

### 前端
- 导入校验结果页新增「🤖 AI 智能校验」按钮
- 校验结果面板：问题分类、严重等级、修复建议
- 支持一键忽略/批量处理

---

## v4.0.0 — 2026-03-25 AI 智能字段配置助手

### 后端
- 新增 `POST /api/ai/field-suggest` — AI 字段配置推荐
- 基于字段名、数据类型、采样数据推荐：语义名、只读/系统字段、枚举值
- 规则引擎内置映射表（覆盖80%+常见字段）+ LLM 增强

### 前端
- 字段配置页新增「🤖 AI 智能推荐」按钮
- 推荐弹窗：逐条展示推荐项（语义名/只读/枚举等）+ 推荐理由
- 支持逐条采纳/全部采纳/仅采纳语义名

### AI 基础设施
- AI 设置页面：9 个大模型平台预设、OpenAI + Claude 双协议
- AIEngine 统一调度：规则引擎 → 统计分析 → LLM 适配器
- 置信度阈值体系：≥0.8 直接展示、0.5-0.8 标黄提醒、<0.5 不展示
- 7 个 AI 功能独立开关

---

## v3.10.0 — 2026-03-25 DELETE 回写支持

### 功能
- 数据浏览页新增操作列，支持标记行为「待删除」
- 导入校验自动检测删除标记
- 差异预览区分：更新行（蓝色）、新增行（绿色）、删除行（红色）
- 回写执行 DELETE（先备份），结果页显示删除数

---

## v3.9.0 — 2026-03-25 INSERT 回写支持
...
## v3.8.0 — 2026-03-25 启动器版本更新检查

### 功能
- Windows 启动器新增版本更新检查（检查 GitHub Releases）
- 有新版本时弹窗提示，可跳转下载
- launcher.py 版本号跟踪

---

## v3.7.0 — 2026-03-25 Inno Setup 安装器

### 功能
- 新增 Inno Setup 安装脚本，生成 Windows .exe 安装程序
- 安装器自动检测/创建桌面快捷方式
- CI 集成：tag 推送自动触发编译 + 生成安装器 + 上传 GitHub Releases
- 安装器自动检测 exe/bat 启动方式降级
