...
### 用户管理（管理员功能）
- 用户列表：查看所有用户、角色、状态、最后登录时间
- 新增用户：指定用户名、显示名、角色
- 编辑用户：修改显示名和角色
- 禁用/启用用户（保护最后一个管理员不可禁用）
- 重置密码
...
### 三角色权限控制
- 管理员（admin）：全部功能 + 系统设置 + 用户管理
- 操作员（operator）：数据浏览、导出、导入、回写、日志
- 只读（viewer）：仅数据浏览和日志查看
- 前端菜单按角色隔离，后端 API 角色校验装饰器（403 拦截越权）
- 操作日志记录实际操作人

---

## v3.5.0 — 2026-03-25 模板安全加固
...
- 主键列已有数据行只读
- 工作表保护（带密码）
- 只读单元格浅灰背景，可编辑单元格白色，底部预留新增数据区浅黄提示
...
## v3.4.0 — 2026-03-25 Windows 版本 + CI 自动发布

### Nuitka 原生编译
- 用 Nuitka 替代 PyInstaller，将 Python 代码编译为原生 C 二进制
- 源码不可反编译，实现强代码保护
- 编译产物可执行文件名：`dataops-server`

### 双平台支持
- **Linux x64**：tar.gz 格式发布包
- **Windows x64**：zip 格式发布包（首次支持 Windows！）
- 两个平台均可独立运行，无需安装 Python/Node.js 环境

### GitHub Actions CI/CD
- 推送 `v*` 格式 tag 自动触发双平台编译
- 编译完成后自动上传到 GitHub Releases
- 发版流程：`git tag v3.4.x && git push --tags` → 自动编译发布

### 仓库拆分
- 公开仓库 `data-ops-workbench`：框架 + 10 个开源插件（GitHub Public）
- 私有仓库 `data-ops-workbench-pro`：全量 19 插件 + AI 引擎（GitHub Private）
- 插件加载器自动扫描机制，私有插件放入 plugins/ 目录即可识别
